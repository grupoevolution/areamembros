...
async function processSale(sale) {
    ...
    for (const item of sale.items) {
        try {
            ...
            if (sale.event === 'approved') {
                ...
                if (offer.is_premium) {
                    summary.premium_sale = true; // recepção VIP usa o tier
    ...
    if (sale.event === 'approved' && summary.granted_product_ids.length > 0) {
        ...
        // RECEPÇÃO VIP: a venda inclui o PLANO DE CHAT (VIP ou Premium)? As
        // modelas configuradas no painel "chegam" espalhadas no tempo (cada
        // uma agendada com atraso sorteado — o worker entrega e manda push).
        try {
            const chatsApi = require('../routes/user-chats');
            if (typeof chatsApi.scheduleVipReception === 'function' && summary.granted_product_ids.length) {
                const { rows: [cp] } = await db.query(
                    `SELECT id FROM products WHERE is_chat_plan = true AND is_active = true ORDER BY id LIMIT 1`);
                if (cp && summary.granted_product_ids.includes(cp.id)) {
                    summary.chat_plan_tier = summary.premium_sale === true ? 'premium' : 'vip';
                    await chatsApi.scheduleVipReception(email, summary.chat_plan_tier);
                }
            }
        } catch (err) { logger.warn('recepção VIP falhou: ' + err.message); }
        ...
        // Comprou o VIP (não o premium): DEPOIS do parabéns, o suporte oferece
        // o upgrade de R$30 com vídeo (config no card "Chamada só no PREMIUM")
        try {
            const chatsApi = require('../routes/user-chats');
            if (summary.chat_plan_tier === 'vip' && typeof chatsApi.deliverUpgradeOfferToSupport === 'function') {
                const dl = await chatsApi.deliverUpgradeOfferToSupport(email);
                for (const s of dl) {
                    try {
                        const { sendChatPush } = require('./chat-worker');
                        const firstBot = (s.messages || [])[0];
        ...
        } catch (err) { logger.warn('upgrade no suporte falhou: ' + err.message); }
    }
...
async function grantAccess({ email, product_id, offer_row_id, gateway, sale_id, sale_amount, sale_net_amount, utm_content }) {
    ...
    if (productType === 'video_call') {
        const accessId = await db.transaction(async (client) => {
            ...
            // Insere novo acesso ativo (utm_content = carimbo de origem da
            // venda — sem ele a pizza "vendas por área" perdia as chamadas)
            const { rows: [newAccess] } = await client.query(
                `INSERT INTO user_access (
                    email, product_id, offer_id, gateway, sale_id, sale_amount, net_amount,
                    status, granted_by, metadata, utm_content
                ) VALUES ($1, $2, $3, $4, $5, $6, $7, 'active', 'webhook', $8::jsonb, $9)
                RETURNING id`,
                [
                    email,
                    product_id,
...
async function revokeAccess({ email, product_id, gateway, sale_id, reason }) {
    ...
    // sale_id NULL só revoga se o acesso nasceu de webhook (linha legada):
    // acesso dado NA MÃO pelo painel (granted_by = usuário do admin) não pode
    // cair porque o cliente gerou um Pix daquele produto e deixou expirar.
    const { rowCount } = await db.query(
        `UPDATE user_access
         SET status = $1,
             revoked_at = NOW(),
         ...
         WHERE LOWER(email) = $4
           AND product_id = $5
           AND status = 'active'
           AND (sale_id = $6 OR (sale_id IS NULL AND granted_by IN ('webhook', 'premium')))`,
        [
            newStatus,
