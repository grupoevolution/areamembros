    ...
    try {
        const { rows } = await db.query(
            `SELECT ua.id, ua.email, ua.gateway, ua.sale_id, ua.offer_id AS offer_row_id
             FROM user_access ua
             JOIN product_offers po ON po.id = ua.offer_id AND COALESCE(po.is_premium, false) = true
             JOIN products p ON p.id = ua.product_id AND p.is_chat_plan = true
             WHERE ua.status = 'active'
               AND (ua.expires_at IS NULL OR ua.expires_at > NOW())
               AND COALESCE(ua.metadata->>'premium', '') <> 'true'`
        );
        for (const r of rows) {
            await db.query(
            ...
            try {
                await grantPremiumExtras({ email: r.email, gateway: r.gateway, sale_id: r.sale_id, offer_row_id: r.offer_row_id });
            } catch (e) { logger.warn(`[premium-backfill] extras de ${r.email} falharam: ` + e.message); }
            logger.info(`[premium-backfill] ${r.email}: selo dourado + extras aplicados`);
        }
        if (rows.length) logger.info(`[premium-backfill] ${rows.length} compra(s) Premium antigas regularizadas`);
