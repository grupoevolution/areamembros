...
async function processSale(sale) {
    ...
    if (sale.event === 'approved' && summary.accesses_granted > 0) {
        ...
        // FUNIL PÓS-VENDA no próprio chat: conversas PAUSADAS no bloco 🔒
        // Paywall deste e-mail RETOMAM daqui (a parte paga do roteiro começa
        // a chegar sozinha) + push "te mandou mensagem".
        try {
            const chatsApi = require('../routes/user-chats');
            if (typeof chatsApi.resumePaywalledChats === 'function') {
                const resumed = await chatsApi.resumePaywalledChats(email);
        ...
        } catch (err) { logger.warn('retomada pós-paywall falhou: ' + err.message); }
