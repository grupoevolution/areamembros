    ...
    if (sale.event === 'pending') {
        try {
            const { rows: ins } = await db.query(
                `INSERT INTO pix_pending_notices (gateway, sale_id, customer_email)
                 VALUES ($1, $2, $3) ON CONFLICT (gateway, sale_id) DO NOTHING RETURNING id`,
                [sale.gateway, sale.sale_id || `sem_id_${Date.now()}`, email]
            );
            if (!ins.length) {
                return { ...summary, message: 'Pix pendente já notificado (dedupe)' };
            }
            // Nome do produto: prefere o cadastrado no painel; senão, o do gateway
            let productName = null;
            try {
                const first = sale.items[0] || {};
                const { rows: [off] } = await db.query(
                    `SELECT p.name FROM product_offers po
                     LEFT JOIN products p ON p.id = po.product_id
                     WHERE po.gateway = $1 AND po.offer_id = $2 LIMIT 1`,
                    [sale.gateway, first.offer_id]
                );
                productName = (off && off.name) || first.product_name || first.offer_name || null;
            } catch (_) {}
    ...
    for (const item of sale.items) {
        try {
            ...
            if (sale.event === 'approved') {
                ...
                if (offer.product_id) summary.granted_product_ids.push(offer.product_id);
