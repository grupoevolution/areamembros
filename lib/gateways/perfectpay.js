...
function mapPerfectPayEvent(saleStatusKey, subscriptionStatusEvent) {
    ...
    // PIX gerado / aguardando pagamento → mensagem de "só falta pagar"
...
function normalizePerfectPayPayload(payload) {
    ...
    // dados de pagamento pro evento 'pending' (PIX copia-e-cola quando vier)
    const payment = {
        method: String(payload.payment_type_enum_key || payload.payment_method || '').toUpperCase() || null,
        pix_code: payload.pix_qr_code || payload.pix_code || payload.qr_code || null,
        pix_url: payload.billet_url || payload.checkout_url || null,
    };
