...
function normalizeKirvanoPayload(payload) {
    ...
    // UTM que veio no link do checkout e a Kirvano DEVOLVE no webhook — é como
    // o sistema sabe QUAL chat vendeu (utm_content = 'chat_<id>'). Busca em
    // vários lugares possíveis do payload (o formato varia).
    const utm = payload.utm || payload.tracking || payload.utms || {};
    const utmContent = utm.utm_content || utm.content || payload.utm_content || null;
    ...
    return {
        ...
        data: {
            ...
            utm_content: utmContent ? String(utmContent).slice(0, 120) : null,
