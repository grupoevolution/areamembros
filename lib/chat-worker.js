...
function startChatWorker() {
    if (timer) return;
    // 5s pra a conversa "pingar" perto do tempo real quando o cliente está FORA
    // dela (mensagens em delay chegam ~no horário + disparam push/banner).
    timer = setInterval(() => { tick().catch(e => logger.warn('[chat-worker] erro: ' + e.message)); }, 5 * 1000);
    setTimeout(() => { tick().catch(() => {}); }, 4 * 1000);
    logger.info('[chat-worker] iniciado — retoma conversas em delay a cada 5s');
}
