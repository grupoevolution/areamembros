...
function previewText(m) {
    if (!m) return 'Você tem uma mensagem nova';
    if (m.type === 'text' || m.type === 'buttons' || m.type === 'cta') return (m.content || 'Mensagem nova').slice(0, 120);
    if (m.type === 'audio') return 'Mandou um áudio';
    if (m.type === 'image') return 'Mandou uma foto';
    if (m.type === 'view_once_image') return 'Mandou uma foto 👀';
    if (m.type === 'view_once_video') return 'Mandou um vídeo 👀';
...
async function tick() {
    let resumeDelayed;
    try { resumeDelayed = require('../routes/user-chats').resumeDelayed; } catch (_) { return; }
...
function startChatWorker() {
    if (timer) return;
    timer = setInterval(() => { tick().catch(e => logger.warn('[chat-worker] erro: ' + e.message)); }, 30 * 1000);
    setTimeout(() => { tick().catch(() => {}); }, 18 * 1000);
    logger.info('[chat-worker] iniciado — retoma conversas em delay a cada 30s');
}

module.exports = { startChatWorker, tick };
