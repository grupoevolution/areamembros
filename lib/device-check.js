...
function deviceCheckMiddleware(req, res, next) {
    ...
    const exemptPaths = [
        ...
        '/manifest.json',
        '/sw.js',
        // Imagens de conteúdo: o otimizador (wsrv.nl) busca com UA de servidor;
        // sem esta isenção ele recebia o 503 falso e o celular baixava a original pesada.
        '/uploads/',
        '/favicon',
        '/robots.txt',
        // Páginas legais (política/termos): o revisor do Meta abre do DESKTOP.
        // São só texto institucional — sem oferta, sem risco de clonagem.
        '/privacidade',
        '/termos',
    ];
    ...
    // ⚠️ PAINEL ADMIN: exige a devkey em QUALQUER aparelho (pedido do dono —
    // antes qualquer celular abria a tela de login do /admin; agora sem o
    // cookie da chave, celular também vê o 503 falso). Desbloqueio: abrir
    // /admin?devkey=CHAVE uma vez no aparelho (cookie de 30 dias, acima).
    // SÓ o caminho /admin — o app do cliente não passa por aqui.
    if (req.path === '/admin' || req.path.startsWith('/admin/')) {
        logger.debug(`/admin sem devkey bloqueado (IP: ${getIp(req)})`);
